...
class WeatherData():
    ...
    def _get_single_variable(self, var, lat, lon):
        lat_idx, lon_idx = self._check_index(lat, lon)
        return var[:,:,lat_idx,lon_idx].reshape(1,-1)
    
    def _get_all_variables(self, lat, lon):
        all_vars = []
        for var in self.all_features:
            all_vars.append(self._get_single_variable(var, lat, lon))
...
w = WeatherData()
print(w.lat.__array__().shape)
print(w.lon.__array__())
print(w.time.__array__())
w._make_grid()
